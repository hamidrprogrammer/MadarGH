...
class WorkViewModel extends BaseViewModel with MyNotificationListener {
  final MyNotification _notification = GetIt.I.get();
  AppState childState = AppState.idle;
  bool newUi = true;

  WorkViewModel(super.initialState) {
...
//     });
  ...
  getData() async {
    ...
    return ""; // Retrieve the saved string
  }
 void getWorkBooks(String userChildId) {
  ...
  void getChildOfUserAge() {
    GetChildOfUserUseCase().invokeAge(MyFlow(flow: (appState) {
      ...
      if (appState.isSuccess) {
        if (appState.getData is List<ChildsItem>) {
          ...
          if (child.isNotEmpty) {
            ...
            // _notification.publish('WorkViewModel', selected);
          }
        }
      }
      refresh();
    ...
    if (jsonString != null) {
      ...
      GeneralReportCardUseCase().invokePcage(
        MyFlow(flow: (appState) {
          print("getGeneralReportCardByPacage");
          print(appState.isSuccess);
           print(appState.getData is WorkBookDetailResponse);
          if (appState.isSuccess &&
